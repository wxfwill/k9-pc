...
class SubTable extends React.Component {
  ...
  fetch(params){
    ...
    let _this = this;
    httpAjax('post',config.apiUrl+'/api/onDuty/dutyUserGroup',{...params}).then((res)=>{
  ...
  handleTableData(content,row,index,colHeader){
    ...
    let obj =  {
      children: typeof content!=="undefined"?<GtableEdit  value={value} userId={userId} rowNumber={index+1}  onChange={this.handleDataChange} udTime={colHeader}></GtableEdit>:'',
      props:{},
    };
    return obj;
  }
  ...
  handleDataChange(options,hide){
    httpAjax('post',config.apiUrl+'/api/onDuty/updateUser',{...options}).then((res)=>{
      if(res.code==0){
        hide();
...
mapPeoples = (arr,type) => {
  ...
  renderCardItem = (item) => {
    const pathname = this.props.pathname.indexOf('app') >= 0? 'app' : 'view'
      ...
      <Table loading={this.state.loading} columns={this.getColumns(item.groupName)} dataSource={item.data} pagination={false}  bordered  onChange={this.handleTableChange}/>
      </div>
  </Card>
  }
    ...
    return (
        ...
        <Collapse defaultActiveKey={["1","2"]}>
            <Panel showArrow={false} header={this.baseHeader("值班室人员")} key="1">
               <span>值班室人员: </span><div style={{display:'inline-table',width:'90%'}}><GtableEdit  value={value} userId={userId} rowNumber={0}  onChange={this.handleDataChange} ></GtableEdit></div>
              </Panel>  
