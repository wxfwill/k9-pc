  ...
  render(){
    ...
    const columns = [{
      ...
      render:(id,record,index)=>{
        return <Badge count={index+1} style={{minWidth: '50px',fontSize:'12px',height:'16px',lineHeight:'16px', backgroundColor: '#99a9bf' }} /> 
      }
    },{
      title: '发病日期',
        ...
        render:(text,record,index)=>{
          return <div>
            <span style={{cursor: "pointer",color:'#1890ff'}} onClick={()=>this.viewDetail(record)}>
              <Link to={{pathname:`/${pathname}/dog/cureView`, query: { dogId: record.dogId,targetText:'查看' }}}><Icon type='eye'   style={{cursor: "pointer",color:'#1890ff',margin:'0 10px'}}/></Link>
            </span>
