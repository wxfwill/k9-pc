...
tMap.prototype.drawingManager = function () {
  ...
  this.Event.addListener(drawingManager, 'markercomplete', function (event) {
    //绘制 marker标记 结束
    ...
    _this.markers = event;
    const center = new qq.maps.LatLng(event.position.lat, event.position.lng);
...
tMap.prototype.getRectPaths = function (aob, distance) {
  ...
  const xb = {lat: maxLat, lng: minLng};
  const db = {lat: maxLat, lng: maxLng};
  const dn = {lat: minLat, lng: maxLng};
  const xn = {lat: minLat, lng: minLng};
...
//圈定目标区域范围
tMap.prototype.createGrid = function (maxLat, minLat, maxLng, minLng, parts) {
  //parts 栅格数
  const _this = this;
  ...
  //网格路径
  const rowStepSize = (maxLat - minLat) / parts;
  const colStepSize = (maxLng - minLng) / parts;
  this.maxLat = maxLat;
  this.minLat = minLat;
  this.maxLng = maxLng;
  this.minLng = minLng;
...
tMap.prototype.getLabelContent = function (argument) {
  ...
  latArr = Array.from(new Set(latArr)).sort(function (a, b) {
    return b - a;
  });
