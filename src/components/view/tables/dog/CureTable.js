...
class CureTable extends React.Component {
  ...
  render() {
    const columns = [{
      ...
      key: 'key',
      render:(data,item,b)=>{
        return <span style={{color:'#1890ff',cursor: 'pointer'}} onClick={this.queryDetail.bind(this,item)}>查看详情</span>
      }
    }];
    return (
      <div>
        ...
        {this.state.showDetail?<CureDetailTabl handleShow={this.handleShow.bind(this)} caption={this.state.detailTitle} changeLeft={this.state.changeLeft}/>:null}              
      </div>
    );
  }
}
