...
class AttendTable extends React.Component {
  ...
  fetch(params = {pageSize: this.state.pageSize, currPage: this.state.currPage}) {
    ...
    React.$ajax
      .postData('/api/leaveRecord/attendanceStatisticsList', {...params})
      .then((res) => {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: function (userId, record) {
          const {statisticsTime} = record;
          return (
            <span
              style={{color: '#1890ff', cursor: 'pointer'}}
              onClick={_this.queryDetail.bind(this, userId, statisticsTime)}>
              查看详情
            </span>
          );
        }
      ...
      <div>
        ...
        {this.state.showDetail ? (
          <AttendDetail
            ...
            userId={this.state.queryId}
            statisticsTime={this.state.statisticsTime}
