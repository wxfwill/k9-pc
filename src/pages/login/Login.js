...
class Login extends Component {
  constructor(props) {
    ...
    const localPwd = JSON.parse(localStorage.getItem('password'));
    const username = typeof localUser === 'object' ? '' : localUser;
    const password = typeof localPwd === 'object' ? '' : localPwd;
    this.state = {
      username: username,
      password: password,
      validate: false,
      loginLoading: false,
      remUser: typeof localUser !== 'object',
      remPwd: typeof localPwd !== 'object'
    };
  }
  componentDidMount() {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        console.log(users);
        const desEncrypt = encodeAesString(JSON.stringify(users));
        const encrypt = encryptRAS();
        React.$ajax.login.postLogin({k: encrypt, d: desEncrypt}).then((res) => {
          if (res && res.code == 0) {
            hide();
            this.setState({loginLoading: false}, () => {
              const {user, menus, token} = res.data;
              const userJson = JSON.stringify(user);
              const munuJson = JSON.stringify(menus);
