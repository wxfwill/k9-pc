    ...
    if (record.editable == true) {
      if (this.props.tabKey == 1) {
        return (
          <div>
            <Select
              defaultValue={this.mapPerformanceCode(text)}
            ...
            <FormItem>
              {getFieldDecorator('startTime', {
                ...
                <DatePicker
                  ...
                  onChange={(text, dateString) => this.onTimerChange(text, dateString, record.id, column)}
                />
              )}
            </FormItem>
          </Form>
            ...
            <FormItem>
              {getFieldDecorator('score', {
                rules: [
                  ...
                  {pattern: /^[0-5]\d*$/, message: '请输入数字且不大于5'}
      ...
      return (
        <Form>
          <FormItem style={{width: '90px'}}>
            ...
            })(<Select style={{width: '100%'}}>{AllUsersOption}</Select>)}
          </FormItem>
        </Form>
      );
    } else {
  ...
  render() {
    ...
    const itemColumns = [
      ...
      },
      {
        ...
        render:
          tabKey == 1 || tabKey == 2 || tabKey == 3 || tabKey == 5
            ? (text, record) => this.renderColumns(text, record, 'subjectId')
            : (text, record) => this.renderInput(text, record, 'content')
      },
      {
        ...
        render:
          tabKey != 5
            ? (text, record) => this.renderTimer(text, record, 'startTime')
            : (text, record) => this.renderTimer(text, record, 'opTime')
      },
      {
        ...
        render:
          tabKey == 1 || tabKey == 2 || tabKey == 4
            ? (text, record) => this.renderTimer(text, record, 'endTime')
            : (text, record) => this.renderAllUsers(text, record, 'examiner')
      },
      {
        ...
        render:
          tabKey != 5
            ? (text, record) => this.renderInput(text, record, 'address')
            : (text, record) => this.renderInput(text, record, 'reason')
      },
      {
        title: '评分(5分)',
        dataIndex: 'score',
        ...
        render: (text, record, index) => {
          ...
          return (
            <div className="editable-row-operations">
              {editable ? (
                <span>
                  <Button onClick={() => this.save(record.id)} size="small" style={{marginRight: '10px'}}>
                    保存
                  </Button>
    ...
    return (
      <div>
        <div style={{marginBottom: '10px'}}>
          ...
          {tabKey == 1 ? <span>（满分4X4X5分=80分，每少一次训练减5分，每半个工作日最多加搜毒搜爆训练分5分）</span> : ''}
          {tabKey == 2 || tabKey == 3 || tabKey == 5 ? (
            <span>
              （两个科目至少训练次数相加不少于16次），每次5分，满分16X5分=80分，每半个工作日最多加刑侦科目训练分5分）
            </span>
          ) : (
            ''
          )}
