...
class StableEdit extends React.Component {
  ...
  edit = () => {
    ...
    this.setState({ editable: true },function(){
    	httpAjax('post',config.apiUrl+'/api/userCenter/getAllUsers',{dutyType:rowNumber==6?rowNumber+1:rowNumber}).then((res)=>{
      	let { data } = res;
  ...
  render() {
    ...
    let {rowNumber} = this.props;
    const children = [];
    optionArr.length>0&&optionArr.map((item,index)=> {
      children.push(<Option key={item.id+"&"+item.groupId+"&"+item.classId} value={item.name}>{item.name}</Option>);
    })
    return (
      <div className="editable-cell"  style={{height:(rowNumber<3 || rowNumber%2>0)?"":"80px"}}>
        {
          editable ?
            <div className="editable-cell-input-wrapper">
              <Select value={value}  style={{width:'100%'}}  mode={rowNumber%2>0?"":"multiple"} onChange={this.handleChange.bind(this)}>
              	{
                  children
