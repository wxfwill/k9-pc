...
class GridTaskTaskInfoListView extends React.Component {
  ...
  fetch(params = {taskId: this.state.taskID}) {
    ...
    httpAjax('post', config.apiUrl + '/api/cmdMonitor/getTaskById', {...params, ...this.state.filter})
      .then((res) => {
        var ti = res.data;
  ...
  render() {
    ...
    const taskReportInfo = ti.taskReportInfo;
    return (
      <div style={{maxHeight: '700px', marginLeft: 18}}>
        ...
        <p>
          <span>上报状态：</span>
          {taskReportInfo ? (taskReportInfo.status == 0 ? '未上报' : '已上报' || '--') : '--'}
        </p>
        <p>
          <span>查缉区域：</span>
          {taskReportInfo ? taskReportInfo.palce || '--' : '--'}
        </p>
        <p>
          <span>分局名称：</span>
          {taskReportInfo ? taskReportInfo.substation || '--' : '--'}
        </p>
        {taskReportInfo &&
          taskReportInfo.commentList.length > 0 &&
          taskReportInfo.commentList.map((comment) => (
            <p key={comment.id}>
        ...
        <p>
          <span>任务反馈：</span>
          {taskReportInfo ? taskReportInfo.remark || '--' : '--'}
        </p>
      </div>
    );
  }
