...
class EditableTableForm extends React.Component{
	...
	renderColumns(text, record, column) {
		...
		let  subjectList= SubjectItems&&SubjectItems.map((item,index)=>{
			return <Option value={item.id} key={index}>{item.subjectName}</Option>
		  })
						...
						<Form>
							<FormItem>
								{getFieldDecorator("subject", {
									...
									<Select defaultValue={record.subjectId} onChange={value=> this.onSelectChange(value, record.id, column)} style={{width:'100%'}}>
										{subjectList}
									</Select>
								)}
							</FormItem>
						...
						<Form>
							<FormItem>
								{getFieldDecorator("item", {
									...
									<Select defaultValue={isFlag?record.itemId:''} onChange={value=> this.onSelectChange(value, record.id, column)} style={{width:'100%'}}>
										{itemList}
									</Select>
								)}
							</FormItem>
				...
				return 	<Form>
					<FormItem>
			          {getFieldDecorator("score", {
			            rules: [{ required: true, message: '请输入考核分数' },{pattern: /^-?[0-9]\d*$/,message:'请输入数字'}],//,{len:1,message:'不得超过两位数'}
			            initialValue:score
			          })(
			...
			SubjectItems.map((sub)=>{
				if(sub.id==value){
					itemList =	sub.performanceCheckItemVOList.map((item,index)=>{
						return <Option value={item.id} key={index}>{item.item}</Option>
					  })
				}
			})
			          ...
			          <div className="editable-row-operations">
			            {
			              editable ?
			                <span>
			                <Button onClick={() => this.save(record.id,record.key)} size='small' style={{marginRight:'10px'}}>保存</Button>
