    ...
    handleSubmit = () => {
        this.props.form.validateFields((err, values) => {
            ...
            if(!err){
                ...
                httpAjax('post',config.apiUrl+'/api/braceletInfo/saveInfo', parms).then((res)=>{
                    if(res.code==0){
...
componentDidMount() {
    ...
    if(VideoId) {
        httpAjax('post',config.apiUrl+'/api/braceletInfo/info', {id: VideoId}).then((res)=>{
            if(res.code==0){
                    ...
                    <Row gutter={24}>
                        ...
                        <FormItem label='颈环编号' {...secondLayout}  hasFeedback>
                            {getFieldDecorator('uniqueNo',{
                             rules: [{ required: true,whitespace:true, message: '请输入颈环编号' },{validator: this.checkNumber}],
                             initialValue:isInitialValue?videoData.uniqueNo :""
                         })(
                            <Input placeholder="颈环编号" onChange={(e) => {this.setState({uniqueNo: e.target.value})}}  disabled={disabled}/>
                          )}
                        </FormItem>
                  </Col>
                  <Col xl={12} lg={12} md={24} sm={24} xs={24}>
              ...
              <Row gutter={24}>
                        ...
                        <FormItem label='警犬' {...secondLayout}  hasFeedback>
                          ...
                          //   rules: [{ required: true,whitespace:true, message: '请选择警犬' },{validator: this.checkNumber}],
                            ...
                            <Select placeholder="警犬"  
                                    optionLabelProp="children" 
                                    showSearch autosize={{ minRows: 2, maxRows: 24 }}
                                    disabled={disabled}
                                    filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                                >
                                    {dogList.map((item) => <Option value={item.id} key={item.id+'_dog'}>{item.name}</Option>)}
                                </Select>
                          )}
                        </FormItem>
