...
class Login extends Component{
  ...
  componentDidMount() {
    // animation(THREE);
    console.log('loginStatus', this.props, this.props.sysActions)
  }
  ...
  handleSubmit(e){
   ...
   _this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        httpAjax('post',config.apiUrl+'/api/userCenter/login',{account:values.userName.trim(),password:values.password.trim()}).then((res)=>{
          ...
          <Form onSubmit={this.handleSubmit.bind(this)} className="login-form">
            <FormItem hasFeedback={this.state.validate}>
              {getFieldDecorator('userName', {
                 ...
                 <Input autoComplete="off"  onChange={this.handleInput.bind(this,'username')} prefix={<Icon type="user" style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder="用户名" className='username' />
              )}
            </FormItem>
            <FormItem hasFeedback={this.state.validate}>
              {getFieldDecorator('password', {
                ...
                <Input  autoComplete="off"onChange={this.handleInput.bind(this,'password')} prefix={<Icon type="lock" style={{ color: 'rgba(0,0,0,.25)' }} />} type="password" placeholder="密码" className='password' />
              )}
            </FormItem>
            <FormItem>
              <Checkbox
             ...
             <Button type="primary" loading={this.state.loginLoading} htmlType="submit" style={{width:'100%'}} className="login-form-button">登录</Button>
          </Form>
