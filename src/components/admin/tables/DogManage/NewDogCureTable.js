...
class DogTable extends Component{
  ...
  fetch(params = {pageSize:this.state.pageSize,currPage:this.state.currPage}){
    ...
    React.$ajax.postData('/api/treatmentRecord/list',{...params}).then((res)=>{
      ...
      this.setState({totalPage: res.data.totalPage,dataSource:[...this.state.dataSource,...res.data.list],loading:false,pagination})
  ...
  renderCardItem = (item) => {
    const pathname = this.props.pathname.indexOf('app') >= 0? 'app' : 'view'
      ...
      const ItemTitle = <div className="card_title">
        <div className="title_h">{item.dogName}病历卡 <i style={{background: item.dealStatus? '#108ee9':'#f50'}}></i><span>{item.dealStatus?'已处理': '未处理'}</span></div>
        <div className="item">
            <span>品种：</span><span>{item.breedName}</span>
        </div>
        <div className="item">
            <span>症状：</span><span>{item.symptom}</span>
        </div>
        <div className="item">
            <span>带犬员：</span><span>{item.trainerName}</span>
        </div>
        <div className="item">
            <span>发病日期：</span><span>{moment(new Date(item.morbidityTime)).format('YYYY-MM-DD')}</span>
        </div>
      ...
      <div className="item_body">
         {item.dealStatus==1 ?
          <div className="body_detail" style={{borderRight: '0'}}>
              <Link style={{ color: '#999999'}} to={{pathname:`/${pathname}/dog/cureView`,  query: { dogId: item.id||record.dogId,targetText:'查看' }}} onClick={() => this.addInfo('view')}><Icon type='eye'   style={{cursor: "pointer",margin:'0 10px'}}/>查看</Link>
          </div>:
          <div><div className="body_detail">
              <Link  to={{pathname:`/${pathname}/dog/cureEdit`, query: { dogId: item.id||record.dogId,targetText:'编辑' }}}>
                  <Icon type='edit'   style={{cursor: "pointer",color: '#999999'}} onClick={()=>this.editInfo(item)}/><span onClick={()=>this.editInfo(item)} style={{margin:'0 10px 0 10px', color: '#999999'}}>编辑</span>
              </Link>
              <Link style={{ color: '#999999'}} to={{pathname:`/${pathname}/dog/cureView`,  query: { dogId: item.id||record.dogId,targetText:'查看' }}} onClick={() => this.addInfo('view')}><Icon type='eye'   style={{cursor: "pointer",margin:'0 10px'}}/>查看</Link>
          </div>
