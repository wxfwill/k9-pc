...
class GridTaskTrackGridTable extends React.Component {
  ...
  getNowTrochoid=(data)=>{
    ...
    httpAjax('post',config.apiUrl+'/api/cmdMonitor/showAppTrochoid',{lastPointTime:trochoiInfo.lastPointTime,taskType:3,taskDetailId:trochoiInfo?trochoiInfo.taskDetailId:''}).then((res) => {
  ...
  setUserList(users){ //设置用户列表,用于查询用户的轨迹
  ...
  render() {
    return (
      <div>
        <Table style={{ maxHeight: '400px',overflowY:'scroll'}} columns={columns} dataSource={this.state.users} 
        onChange={this.handleTableChange} onRowClick={this.handonRowClick} size="small" pagination={false}/>
      </div>
    );
  }
}
