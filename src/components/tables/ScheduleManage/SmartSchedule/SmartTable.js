...
class SmartTable extends React.Component {
        ...
        function () {
          if (_this.state.percent == 110) {
            ...
            if (typeof limitData === 'undefined') {
              _this.fetch(
                {startDate: _this.props.filter.startTime, endDate: _this.props.filter.endTime, currPage: 1},
                true
              );
            } else {
  ...
  fetch(params, isFirst) {
    const _this = this;
    const reqUrl = isFirst ? config.apiUrl + '/api/onDuty/genDuty' : config.apiUrl + '/api/onDuty/getDutyByPage';
  ...
  handleData(pagination, data, callBack) {
    ...
    const tableHeader = tableData.dutyDates.shift(0);
    const columnsHeader = [
      {
        userName: '带班领导',
        userId: 2
    ...
    tableData.dutyDates.length > 0 &&
      tableData.dutyDates.forEach((item, index) => {
        item.key = index + 'key';
    ...
    const obj = {
      children:
        typeof day !== 'undefined' ? (
          <StableEdit
            value={day.name}
            userId={day.userId + '&' + day.groupId + '&' + day.classId}
            rowNumber={index + 1}
            onChange={this.handleDataChange}
            udTime={colHeader.userId}></StableEdit>
        ) : (
          ''
        ),
      props: {}
  ...
  handleDataChange(options, hide) {
    httpAjax('post', config.apiUrl + '/api/onDuty/updateDuty', {...options})
      .then((res) => {
        if (res.code == 0) {
          hide();
      ...
      },
      {
        ...
        render: (monday, row, index) => {
          ...
          if (index < 8) {
            return {
              children:
                typeof monday !== 'undefined' ? (
                  <StableEdit
                    value={monday.name}
                    userId={monday.userId + '&' + monday.groupId + '&' + monday.classId}
                    udTime={tableHeader.monday.userId}
                    rowNumber={index + 1}
                    onChange={this.handleDataChange}
                  />
                ) : (
    ...
    return (
      <div className="smart-table">
        <Progress
          ...
          className="progress"
          width={150}
          style={{display: this.state.percent == 110 ? 'none' : 'block'}}
        />
        {this.state.percent > 100 && this.state.showTable ? (
          <Table
            loading={this.state.loading}
            columns={this.getColumns(this.state.tableHeader)}
