...
class RoundsTrack extends Component{
  ...
  fetch(params = {id:this.props.match.params.taskID}){
    ...
    let url = '/api/cmdMonitor/emergencyDeploymentPlanInfo', type = '';
    ...
    httpAjax('post',config.apiUrl+url,{...params,...this.state.filter}).then((res)=>{
      ...
      if(!type) {
        ...
        //根据指定的范围调整地图视野
        me.TMap.map.setCenter(new qq.maps.LatLng(me._taskInfo.drawShapeDTO.coord.lat,me._taskInfo.drawShapeDTO.coord.lng))
      //  me.TMap.map.fitBounds(circle.getBounds()); 
      }else {
        me.TMap.drawingPolygon(res.data.drawShapeDTO.latLngArr, 'asd');
        me.TMap.map.setCenter(new qq.maps.LatLng(res.data.drawShapeDTO.latLngArr[0].lat, res.data.drawShapeDTO.latLngArr[0].lng))//根据指定的范围调整地图视野
      }
  ...
  beginOrStopTask=(type)=>{
    ...
    httpAjax('post',config.apiUrl+url,{id: this.props.match.params.taskID}).then((res)=>{
      this.setState({
        taskStatus:status
      })
      message.success(msg);
  ...
  //开启视频设备
  startVideo=(id)=>{
    httpAjax('post',config.apiUrl+'/api/sdjw/startVideo',{id:id}).then((res)=>{
      if(res.code==0){
        this.getPlayUrl(id);
      }
  ...
  getPlayUrl=(id)=>{
    ...
    httpAjax('post',config.apiUrl+'/api/sdjw/getPlayUrl',{id:id}).then((res)=>{
      if(res.code==0){
        ...
        if(res.data.url==''){
          ...
          document.getElementById("playerDiv").innerHTML='<div  id="player" style="display:block;width:450px;height:320px;margin-bottom:100"></div>';
          this.player=null;
          if(this.player){
            this.player.load({
              sources: [
            ...
            this.player = new flowplayer ("#player" , { 
              ...
              clip  : { 
                provider: 'rtmp',
                sources : [ 
                  {  type :'video/flash',src:res.data.url  } 
                ],
              } ,
  ...
  //判断视频路径是否过期
  heartbeat=(id)=>{
    httpAjax('post',config.apiUrl+'/api/sdjw/heartbeat',{id:id}).then((res)=>{
  ...
  showUAV=()=>{
    if(this.UAVplayer==null){
      httpAjax('post',config.apiUrl+'/api/basicData/liveUrl',{}).then((res)=>{
        if(res.code==0){
  ...
  render(){
    ...
    let type = '', title=<span>紧急调配</span>;
    let videoStr ='';
    let UAVStr='';
    let stopStr=  taskStatus<2?
                    taskStatus==0?
    ...
    if(this.props.location.query) {
      type = this.props.location.query.type
      type=='duty' ? UAVStr = <Tag color="#108ee9" onClick={() => this.showUAV()} style={{float:'right'}} >无人机视频</Tag>:null;
      type=='duty' ? videoStr = <Tag color="" onClick={() => this.closeVideo()} style={{float:'right'}} >关闭视频</Tag>:null;
      type=='duty' ? title=<span>日常巡逻</span>:title=<span>紧急调配</span>;
    }
    const topStr=<div style={{marginTop:2}}>{title}{stopStr}{videoStr}{UAVStr}</div>
    return (
      <div>
        <div className="GridRaid" style={{left:collapsed?"92px":"212px"}}>
          ...
          <Row gutter={24} id="container">
            ...
            <Card title={topStr} bordered={false} 
              extra={<Tag color="#2db7f5" 
              onClick={this.props.history.goBack}>
              <Icon type="rollback" />返回</Tag>} bordered style={{ width: cardWidth,position:'absolute',top:'0',right:'0',bottom:10, zIndex:'9999',height: '100%'}}>  
              <Search
                placeholder="地点搜索"
              ...
              <Tabs defaultActiveKey="1">
                <TabPane tab="查询轨迹" key="1">
                    <GridTable taskID={this.props.match.params.taskID} taskStatus={taskStatus}  type={type} showVideo={this.showVideo}  drawTrace={this.drawTrace}/> 
                </TabPane>
        ...
        <div id="playerDiv"  style={{left:collapsed?"92px":"212px",position: 'absolute',top:81,display:display}}>
            <div  
              style={{display:'block',width:'450px',height:'320px',marginBottom:'100' }}
              id="player"> 
            </div> 
        </div>

        <div id="UAVDiv"  style={{left:collapsed?"92px":"212px",position: 'absolute',bottom:20,display:UAVdisplay}}>
            <div  
              style={{display:'block',width:'450px',height:'320px',marginBottom:'100' }}
              id="UAVDplayer"> 
            </div> 
