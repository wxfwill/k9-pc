...
class CommandEdit extends React.Component {
  ...
  handleSubmit = () => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        httpAjax('post', config.apiUrl + '/api/command4w/save', param)
          .then((res) => {
            if (res.code == 0) {
  ...
  componentDidMount() {
    ...
    if (id) {
      httpAjax('post', config.apiUrl + '/api/command4w/queryDetail', { id: id })
        .then((res) => {
          if (res.code == 0) {
            const peoValue = res.data.userVOS.map((t) => t.name);
            const targetKeys = res.data.userVOS.map((t) => t.id);
            this.setState({
              commandDetail: res.data,
              ...res.data,
              peoValue: peoValue.join(','),
                      ...
                      <FormItem label="记录时间" {...secondLayout}>
                        {getFieldDecorator('recordTime', {
                          ...
                          initialValue: isInitialValue
                            ? commandDetail.recordTime
                              ? commandDetail.recordTime && moment(new Date(commandDetail.recordTime))
                              : ''
                            : '',
                      ...
                      <Col xl={12} lg={12} md={24} sm={24} xs={24}>
                        <FormItem label="任务执行时间" {...secondLayout} hasFeedback>
                          {getFieldDecorator('taskTime', {
                            ...
                            initialValue: isInitialValue
                              ? commandDetail.taskTime
                                ? commandDetail.taskTime && moment(new Date(commandDetail.taskTime))
                                : ''
                              : '',
                          })(<DatePicker disabled={disabled} />)}
                        </FormItem>
