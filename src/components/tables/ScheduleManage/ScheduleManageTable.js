...
class ScheduleManageTable extends React.Component {
  ...
  resetRender = (searchWeek) => {
    const tHeaderData = searchWeek.dutyDates.shift();
    const rHeaderArr = [
      {
        title: '带班领导',
        id: 2
  ...
  onCellChange = (key, getTimer, dataIndex, childrenIndex) => {
    ...
    const reqUrl = config.apiUrl + '/api/onDuty/updateDuty';
    const groupId = dataIndex == 4 ? childrenIndex && childrenIndex.groupId : dataIndex + 1;
    let options = {
    ...
    if (dataIndex == 8 || (key != '' && editDutyUser != (key && key.split('&')[1]))) {
      httpAjax('post', reqUrl, options)
        .then((res) => {
          if (res.code == 0) {
            message.success('更改成功');
            if (localStorage.getItem('getScheduleOption')) {
              const getScheduleOption = JSON.parse(localStorage.getItem('getScheduleOption'));
            ...
            children:
              typeof text === 'undefined' ? (
                ...
                <EditableCell
                  ...
                  index={index}
                  rowHeader={record}
                  onChange={(x) => this.onCellChange(x, dataSourceTitle.monday.userId, index, childrenText)}
                />
              ),
            props: {}
          };
            ...
            children:
              typeof text === 'undefined' ? (
                ...
                <EditableCell
                  ...
                  index={index}
                  rowHeader={record}
                  onChange={(x) => this.onCellChange(x, dataSourceTitle.tuesday.userId, index, childrenText)}
                />
              ),
            props: {}
          };
            ...
            children:
              typeof text === 'undefined' ? (
                ...
                <EditableCell
                  ...
                  index={index}
                  rowHeader={record}
                  onChange={(x) => this.onCellChange(x, dataSourceTitle.wednesday.userId, index, childrenText)}
                />
              ),
            props: {}
          };
            ...
            children:
              typeof text === 'undefined' ? (
                ...
                <EditableCell
                  ...
                  index={index}
                  rowHeader={record}
                  onChange={(x) => this.onCellChange(x, dataSourceTitle.thursday.userId, index)}
                />
              ),
            props: {}
          };
            ...
            children:
              typeof text === 'undefined' ? (
                ...
                <EditableCell
                  ...
                  index={index}
                  rowHeader={record}
                  onChange={(x) => this.onCellChange(x, dataSourceTitle.friday.userId, index)}
                />
              ),
            props: {}
          };
          ...
          const children =
            typeof text === 'undefined' ? (
              ...
              <EditableCell
                ...
                index={index}
                rowHeader={record}
                onChange={(x) => this.onCellChange(x, dataSourceTitle.saturday.userId, index)}
              />
            );
          ...
          const children =
            typeof text === 'undefined' ? (
              ...
              <EditableCell
                ...
                index={index}
                rowHeader={record}
                onChange={(x) => this.onCellChange(x, dataSourceTitle.sunday.userId, index)}
              />
            );
          return renderContent(text, record, index, children);
        }
  ...
  //导出
  expWeekDuty = () => {
    const dateString = localStorage.getItem('ChangeWeek');
    ...
    return (
      <div>
        <div className="table-operations" style={{textAlign: 'left'}}>
          <Button type="primary">
            <Link to="/app/duty/dutyWeekSchedule">智能排班</Link>
          </Button>
          &nbsp;&nbsp;
