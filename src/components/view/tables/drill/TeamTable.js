...
class SubTable extends React.Component {
  ...
  fetch(params = {pageSize:this.state.pageSize,currPage:this.state.currPage}){
    ...
    httpAjax('post',config.apiUrl+'/api/train/listTeamUserPage',{...params}).then((res)=>{
...
deletePlan = (item) => {
    httpAjax('post',config.apiUrl+'/api/train/deleteTeamByIds',{ids: [item.id]}).then((res)=>{
        if(res.code == 0) {
            message.info('删除成功！');
            this.setState({
                currPage:1,
    ...
    const pathname = this.props.pathname.indexOf('app') >= 0? 'app' : 'view'
      ...
      const ItemTitle = <div className="card_title">
        <div className="title_h" style={{width: '250px', overflow: 'hidden',textOverflow: 'ellipsis',whiteSpace: 'nowrap'}}>
            <Tooltip   placement="topLeft" title={item.subjectName}>队名： {item.name}</Tooltip>
        </div>
        <div className="item">
            <span>队长：</span>
        ...
        <div className="item" style={{width: '250px', overflow: 'hidden',textOverflow: 'ellipsis',whiteSpace: 'nowrap'}}>
            ...
            <span>队员：</span><span>{item.members && this.mapPeoples(item.members)}
