...
class RegisterDetail extends Component{
  ...
  fetch(record,checkDate){
    httpAjax('post',config.apiUrl+'/api/performanceCheck/performanceCheckInfo',{userId:record.userId,checkDate:checkDate}).then((res)=>{
      ...
      this.setState({dataSource:res.list,record:record,autonomyData:autonomyData,automaticData:automaticData,checkDate:checkDate})
		...
		<div className="DutyComponent">
            ...
            <Collapse defaultActiveKey={['1']} onChange={this.callback}>
              ...
              <Panel header={<Tag color="#2db7f5">自主打分</Tag>} key="3">
                    ...
                    >
                      <Table dataSource={autonomyData.trainCheck} columns={autonomyColumns} pagination={false}  className='performanceTableForm'/>
                    </Card>
                    <Card
                      style={{ marginTop: 16 }}
                      type="inner"
                    ...
                    >
                      <Table dataSource={autonomyData.dailyManage} columns={autonomyColumns} pagination={false} className='performanceTableForm'/>
                    </Card>
              </Panel>
            </Collapse>
          </Card>
