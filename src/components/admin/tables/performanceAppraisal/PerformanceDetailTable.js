...
class PerformanceDetailTable extends Component {
	...
	fetch(params){
		...
		let  {baseDataSource}=this.state,baseDataArr=[];
		let obj={};
  	...
  	itemChange=(value)=>{
  		...
  		const params={
			...
			let urlArr = ["listCheckSearch","listCriminalInvestigation","listCheckTrain","listCheckDogUse","listCheckDaily"];
				  ...
				  <Card title={this.state.title}>
				  	<Collapse defaultActiveKey={['1','2']}>
					    <Panel showArrow={false} header={this.getTitle('基础信息')} key="1">
                    		<Table loading={loading} columns={baseColumns} dataSource={baseDataSource}  pagination={false} bordered rowKey='id' />							
					    </Panel>
					    <Panel showArrow={false} header={this.getTitle('考核项目')} key="2">
						    <Tabs defaultActiveKey='1' onChange={this.itemChange}>
						    	<TabPane tab='第一部分：搜毒搜爆科目训练' key='1'>
						    		<PerformanceItem addPerformItem={this.addPerformItem} itemData={performceData}  tabKey={tabKey}  getItemData={this.getItemData} />
						    	</TabPane>
