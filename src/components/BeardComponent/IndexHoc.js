...
const getBreadcrumb = ({ flattenRoutes, curSection, pathSection }) => {
  ...
  if (matchRoute) {
    ...
    return matchRoute.name ? <span path={matchRoute.path}>{matchRoute.name || curSection}</span> : null;
  }
  // 对于routes表中不存在的路径
  // 根目录默认名称为首页.
  const defaultRootPageName = '首页';
  return <span path={pathSection}>{pathSection === '/' ? defaultRootPageName : curSection}</span>;
};
