...
class AddPlan extends Component {
    ...
    //获取训练人员详情
    getPlanInfo=(params={ pageSize: this.state.pageSize,
        currPage: this.state.currPage,
        planId:this.state.planId})=>{
            httpAjax('post', config.apiUrl+'/api/train/getTrainRecord',params).then((res)=>{
                const data = res.data;
    ...
    render() {
        const {disabled, typeOption, places, peoples, isDetail, location, reportArr,loading,dataSource,pagination,placeType,trainList } = this.state;
            ...
            <div>
                <Row gutter={24}>
		            <Col span={24}>
			          	<Card title='训练计划' bordered={true}>
			          		<Col xxl={16} xl={22} lg={24} md={24} sm={24} xs={24}>
							               	...
							               	<FormItem label='训练科目：' {...secondLayout} >
							               		{getFieldDecorator('subjectId',{
													...
													<Select disabled={disabled}>
                                                        {typeOption.map((item) => <Option value={item.id} key={item.id+'typeOption'}>{item.name}</Option>)}
                                                    </Select>
							               		)}
							              </FormItem>
							            </Col>
                                               ...
                                               {getFieldDecorator('userIds',{
                                                ...
                                                initialValue:editItem? editItem.userIds.split(','):[]
                                                  ...
                                                  <Select disabled={disabled} mode="multiple" onChange={this.selectPeoples}>
                                                    {peoples.map((item) => <Option value={item.id+''} key={item.id+'place'}>{item.name}</Option>)}
                                                  </Select>
                                                )}
                                            </FormItem>
                                        </Col>
                                           ...
                                           <FormItem label='场地类型：' {...secondLayout}  >
                                                {getFieldDecorator('placeType',{
                                                    ...
                                                    <Select disabled={disabled} mode="single" onChange={this.changePlace}>
                                                        <Option value={''} key={1}>请选择场地类型</Option>
                                                        <Option value={1} key={2}>基地内</Option>
                                                        <Option value={2} key={3}>基地外</Option>
                                                    </Select>
                                                )}
                                            </FormItem>
                                        </Col>
                                  ...
                                  {/* <Row gutter={24}>
                                    ...
                                    <Row gutter={24}>
                                       {placeType&&placeType ==2? <Col xl={12} lg={12} md={24} sm={24} xs={24}>
                                            <FormItem label={'训练场地'} {...secondLayout} hasFeedback>
                                                {getFieldDecorator('location', {
                                                  ...
                                                  <Input placeholder="训练场地" disabled={true} addonBefore={<Icon type="plus" style={{ cursor: 'pointer' }} onClick={this.addCoord.bind(this)} />} />
                                                )}
                                            </FormItem>
							            </Col>:null}
                                        {placeType&&placeType ==1?  <Col xl={12} lg={12} md={24} sm={24} xs={24}>
                                           <FormItem label='训练场地：' {...secondLayout}  >
                                                {getFieldDecorator('placeId',{
                                                    ...
                                                    <Select disabled={disabled} mode="single" onChange={this.changePlaceId}>
                                                        {trainList.map((item) => <Option value={item.id+''} key={item.id+'place'}>{item.name}</Option>)}
                                                    </Select>
                                                )}
                                            </FormItem>
                                        </Col>:null}
                                        ...
                                        <Row gutter={24}>
                                            <Col xl={12} lg={12} md={24} sm={24} xs={24}>
                                               <FormItem label='发布时间' {...secondLayout}  >
                                                   <Input disabled={true} value={editItem && editItem.publishDate ? moment(editItem.publishDate).format("YYYY-MM-DD HH:mm:ss") : '----'} />
                                                </FormItem>
                                            </Col>
                                            <Col xl={12} lg={12} md={24} sm={24} xs={24}>
                                    ...
                                    <Row>
                                        ...
                                        <Button type="primary" htmlType='submit' onClick={() => this.handleSubmit('publish')}>发布</Button>
                        ...
                        {
                            !disabled ?
                              null :<Card title='训练详情' bordered={true}>
                                       <Table  loading={loading} columns={columns}  size="small" dataSource={dataSource} pagination={pagination} onChange={this.handleTableChange} bordered />  
                                    </Card>
                        }
