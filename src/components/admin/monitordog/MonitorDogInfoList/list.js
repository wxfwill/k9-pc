...
class HouseList extends Component {
  ...
  fetch(params = {pageSize: this.state.pageSize, currPage: this.state.currPage}) {
    httpAjax('post', config.apiUrl + '/api/dogRoom/monitor', {...params})
      .then((res) => {
  ...
  ajaxGetDogsInHouse = () => {
    ...
    if (dogIds) {
      httpAjax('post', config.apiUrl + '/api/dogRoom/getDogStatusByIds', {dogIds: dogIds.join(',')})
        .then((res) => {
          //   let backState = { "1": "0", "2": "1", "3": "0", "9": "", "94": "1" };
          const backState = res || {};
          const sourceData = this.state.listByLoad.data;
          const targetData = [];
          sourceData.map((item) => {
            const temp = item.dogListVOList;
            const obj = {};
            let i = 0;
            obj.houseName = item.houseName; // 栋数
            for (const j in backState) {
  ...
  renderCard = (item, index) => {
    return (
      <Card style={{borderRadius: 5, minHeight: 155}} key={index} hoverable bodyStyle={{padding: '24px 6px'}}>
        <Row>
          <Col xs={24} sm={24} md={5} lg={5} xl={5} xxl={5}>
            <a href={item.videoUrl}>
              <img
                src={config.apiUrl + `/api/dog/img?id=${item.id}&t=${new Date().getTime()}`}
              ...
              <Col xs={9} sm={9} md={10} lg={10} xl={10} xxl={10}>
                <a href={item.videoUrl}>
                  <span style={{fontWeight: 800, fontSize: 13, color: '#555'}}>
                    {item.dogName || '-'} <Icon style={{color: '#1890ff'}} type="video-camera" />
                  </span>
                </a>
              </Col>
              <Col
                ...
                xl={14}
                xxl={14}
                title={item.isExistHouse === '0' ? '未知' : item.isExistHouse === '1' ? '在犬舍' : '离开中'}
                style={{textAlign: 'right'}}>
                {item.isExistHouse === '0' ? (
                  <Badge status="default" text={item.houseRoomStr || '-'} />
                ) : item.isExistHouse === '1' ? (
                  <Badge status="success" text={item.houseRoomStr || '-'} />
                ) : (
                  <div className="roundScale"></div>
                )}{' '}
              </Col>
            </Row>
  ...
  render() {
    ...
    // 渲染宿舍
    const rendrHouse = data.map((item, i) => {
      return (
        <div key={i} style={{background: '#fbfbfb', padding: 24}}>
          <span style={{fontWeight: 800}}>
            <i className="iconfont icon-sushe2x"></i> {item.houseName || '-'}犬舍（
            {item.dogListVOList ? item.dogListVOList.length : '-'}）
          </span>
