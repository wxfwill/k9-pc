...
class AddHoliday extends Component {
    ...
    render() {
		...
		const holidayTypesOption= holidayTypes&&holidayTypes.map((item,index)=>{
			return <Option value={item.typeId} key={index}>{item.typeName}</Option>
		  })
        return(
            <Row gutter={24}>
		            <Col span={24}>
                                        ...
                                        <Col xl={16} lg={12} md={24} sm={24} xs={24}>
										  <FormItem label='选择年份' {...firstLayout}  >
												{getFieldDecorator('leaveYear',{
													...
													>
														{years.map((item) => <Option value={item.id+''} key={item.id+'_peo'}>{item.name}</Option>)}
													</Select>
												)}
											</FormItem>
							            </Col>     
							      ...
							      <Row gutter={24}>
                                        ...
                                        <Col xl={16} lg={12} md={24} sm={24} xs={24}>
                                            <FormItem label='天数设定' {...firstLayout}  >
                                                ...
                                                initialValue:record&&record.totalDay||''
                                                    ...
                                                    <Input placeholder="请输入天数" autosize={{ minRows: 2, maxRows: 24 }} disabled={this.state.disabled}/>
                                                )}
                                            </FormItem>
							            </Col>     
                                    </Row>	            						            				                    					            						            						            						            						            						            						            						            						            						            	
                                        ...
                                        <Col xl={16} lg={12} md={24} sm={24} xs={24}>
                                            <FormItem label='选择人员' {...firstLayout}  >
                                                ...
                                                initialValue:this.state.peoValue||''
                                                    ...
                                                    >
                                                    <Input placeholder="请选择人员" value={this.state.peoValue} disabled={true} addonBefore={<Icon type="plus" style={{ cursor: 'pointer' }} onClick={this.addPeople.bind(this)} />} />
                                                    </Tooltip>
                                                )}
                                            </FormItem>
							            </Col>     
                                    ...
                                    <Row>
                                        <Col span={24} style={{ textAlign: 'center',marginTop:'40px' }}>
                                     	{disabled ?null : <Button type="primary" htmlType='submit' onClick={() => this.handleSubmit('publish')}>保存</Button>}   
                                        </Col>
                                    </Row> 
