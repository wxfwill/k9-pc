...
class DogTable extends Component {
  ...
  componentDidMount() {
    ...
    const params = {pageSize: this.state.pageSize, currPage: this.state.currPage};
  ...
  fetch(params = {pageSize: this.state.pageSize, currPage: this.state.currPage}) {
    httpAjax('post', config.apiUrl + '/api/dog/listData', {...params})
      .then((res) => {
  ...
  renderCard = (item, index) => {
    return (
      <Col key={index}>
        <Card
          style={{borderRadius: 5, paddingBottom: 1}}
          bodyStyle={{padding: '24px 6px'}}
          hoverable
          actions={[
            <span onClick={() => this.editInfo(item)}>
              {' '}
              <Link to={{pathname: '/app/dog/infoEdit', query: {dogId: item.id, targetText: '编辑'}}}>
                <Icon type="edit" /> 修改{' '}
              </Link>
            </span>,
            ...
            <span onClick={() => this.viewDetail(item)}>
              {' '}
              <Link to={{pathname: '/app/dog/infoView', query: {dogId: item.id, targetText: '查看'}}}>
                <Icon type="eye" /> 查看{' '}
              </Link>
            </span>
          ]}>
          <Row style={{minHeight: 140}}>
            <Col xs={24} sm={24} md={6} lg={6} xl={6} xxl={6}>
              <img
                ...
                style={{width: '80px', height: '80px', borderRadius: 5}}
                alt={item.name}
              />
            </Col>
              ...
              <Col xs={24} sm={24} md={18} lg={18} xl={18} xxl={18}>
                <Row className="cardsList-info">
                  <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                    <span onClick={() => this.viewDetail(item)}>
                      <Link
                        to={{pathname: '/app/dog/infoView', query: {dogId: item.id, targetText: '查看'}}}
                        style={{color: '#666'}}>
                ...
                <table className="card_title ">
                  ...
                  <tr>
                    <td>带犬员：</td>
                    <td>{(item.trainerName || '-') + '-' + (item.trainerNumber || '-')}</td>
                  </tr>
                  <tr>
                    <td>芯片号：</td>
                    <td> {item.chipCode || '-'}</td>
                  ...
                  <tr>
                    <td>服役单位：</td>
                    <td>
                      <span>{item.serviceUnitName || '-'}</span>
                    </td>
                  </tr>
                </table>
              </Col>
      ...
      <div style={{background: '#fbfbfb', padding: 24}}>
        <div style={{marginBottom: '20px'}}>
          <Button type="primary" style={{marginRight: '20px'}} onClick={this.addInfo}>
            <Link to={{pathname: '/app/dog/infoAdd', query: {targetText: '新增'}}}>
              {' '}
              <Icon type="plus-circle-o" /> 新增犬只
            </Link>
          </Button>
