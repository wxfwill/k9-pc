...
class ExecutePlan extends Component{
 ...
 getDogs = () => {
    this.props.form.validateFields((err,values)=>{
        if(!err) {
            httpAjax('post',config.apiUrl+'/api/dog/queryByCreateTime',{startTime: values.startTime.format('x'), endTime: values.endTime.format('x') }).then(res=>{
                if(res.code == 0) {
			          		...
			          		<Col xxl={16} xl={22} lg={24} md={24} sm={24} xs={24}>
			          			<Form className="ant-advanced-search-form">
								      	...
								      	<Col xl={12} lg={24} md={12} sm={24} xs={24} style={{width: '100%'}}>
										   <Form.Item label='选择犬只' {...secondLayout}>
								               {getFieldDecorator('startTime',{
                                                ...
                                                <Button style={{margin: '0 25px'}} type="primary" onClick={this.getDogs} >查询犬只</Button>
							              	</Form.Item>
							            </Col>
                                    ...
                                    <Row>
                                        ...
                                        <Button type="primary" htmlType='submit' onClick={this.submint}>提交</Button>
