...
class Breedinfo extends Component {
  ...
  renderDogList = (type = 'maleDog', dogInfo) => {
    if (type == 'maleDog') {
      return [
        <img src={`${config.apiUrl}/api/dog/img?id=${dogInfo.id}`} />,
  ...
  renderExtra = (item) => {
    ...
    if (breed) {
      return [
        <Link to={{ pathname: '/app/dog/breedEdit', query: { editItem: item } }}>
          <Button type="primary" style={{ marginRight: 10 }}>
            编辑信息
          </Button>
        </Link>,
        <Popconfirm title="确认删除此信息?" onConfirm={() => this.deleteItem(item)}>
          <Button>删除</Button>
        </Popconfirm>,
      ];
    } else {
      return [
        //<Link to={{pathname: '/app/dog/reproduceEdit', query: {editItem: item }}}><Button  type='primary' style={{marginRight: 10}}>编辑繁育信息</Button></Link>,
        <Link to={{ pathname: `/app/dog/reproduceEdit/${item.id}`, query: { editItem: item } }}>
          <Button type="primary" style={{ marginRight: 10 }}>
            编辑幼犬信息
          </Button>
        </Link>,
    ...
    if (breed) {
      return (
        <div className="card_right" key={index}>
          ...
          <div className="breed_l">
            母犬发情时间：{item.estrusTime ? moment(item.estrusTime).format('YYYY-MM-DD') : '--'}
          </div>
        </div>
      );
    } else {
      return (
        <div className="card_right reproduce_c_right" key={index}>
          <div>
            <span className="reproduce_tag">
              出生日期：{item.birthday ? moment(item.birthday).format('YYYY-MM-DD') : '--'}
            </span>
          ...
          <div>{this.mapDogChild(item.pupInfos)}</div>
        </div>
      );
    }
  };
        ...
        <Col xl={24} lg={24} md={24} sm={24} xs={24} key={index}>
          ...
          >
            <div className="card_body" key={index}>
              ...
              <div
                className="card_left"
                style={{ position: 'absolute', top: 0, right: 0, border: 'none', borderLeft: '1px solid #ccc' }}
              >
                {breed
                  ? this.renderDogList('femaleDog', item.femaleDog ? item.femaleDog : { id: item.femaleDogId })
                  : this.renderDogList('maleDog', item.maleDog)}
      ...
      <div className="DutyComponent Officer breedInfo">
        <div style={{ marginBottom: '20px' }}>
          <Button type="primary" style={{ marginRight: '20px' }}>
            <Link to={{ pathname: `/app/dog/${breed ? 'breedAdd' : 'reproduceAdd'}`, query: { targetText: '新增' } }}>
              {breed ? '新增配种记录' : '新增繁殖记录'}
            </Link>
          </Button>
        </div>
