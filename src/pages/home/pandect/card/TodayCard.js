...
class TodayCard extends Component{
  ...
  componentWillMount() {
    ...
    httpAjax('post',config.apiUrl+"/api/overView/todayLog").then((res)=>{
       let { data,code } = res;
       if(0==code){
        this.setState({
        ...
        <Card title={"今日动态"} className="layout-card" bordered={false}>
              ...
              >
    					{this.state.listData.length>0?this.state.listData.map(function(item,index){
    						return(
                <Tooltip placement="topLeft" title={<span>【{item.type}】{item.time}{item.content}</span>}  key={index}>
    							<li key={index}>【{item.type}】{item.time}{item.content}</li>
                </Tooltip>
    						)
    					}):null}
    				</ul>
