...
class SubTable extends React.Component {
  ...
  fetch(params = {pageSize:this.state.pageSize,currPage:this.state.currPage}){
    ...
    httpAjax('post',config.apiUrl+'/api/train/listPlanPage',{...params}).then((obj)=>{
      let res=obj.data;
      const pagination = { ...this.state.pagination };
      pagination.total =res.totalCount;
      pagination.current = res.currPage;
      pagination.pageSize = res.pageSize;
      this.setState({data:[...this.state.data,...res.list],loading:false,pagination, totalPage: res.totalPage})
...
deletePlan = (item) => {
    httpAjax('post',config.apiUrl+'/api/train/deleteByIds',{ids: [item.id]}).then((res)=>{
        if(res.code == 0) {
            message.info('删除成功！');
            this.setState({
                currPage:1,
  ...
  renderCardItem = (item) => {
    const pathname = this.props.pathname.indexOf('app') >= 0? 'app' : 'view'
      ...
      const ItemTitle = <div className="card_title">
        <div className="title_h" style={{width: '250px', overflow: 'hidden',textOverflow: 'ellipsis',whiteSpace: 'nowrap'}}>
        ...
        <div className="item">
            <span>训练时间：</span><span>{moment(item.trainDate).format('YYYY-MM-DD')}</span>
        </div>
        <div className="item" style={{width: '250px', overflow: 'hidden',textOverflow: 'ellipsis',whiteSpace: 'nowrap'}}>
            <Tooltip title={item.location ? item.location : '--'}>训练场地： {item.location ? item.location : '--'}</Tooltip>
        </div>
        <div className="item">
            <span>训练说明：</span><span>{item.remark}</span>
        </div>
        <div className="item">
            <span>发布时间：</span><span>{item.saveStatus == 1 && item.publishDate ? moment(item.publishDate).format('YYYY-MM-DD HH:mm:ss') : '----'}</span>
        </div>
        <div className="item">
            <span>发布人员：</span><span>{item.operator ? item.operator : '--'}</span>
        </div>
    </div>

      return  <Card title={ItemTitle} 
        key={item.id+'card'}
      ...
      <div className="item_body">
         {
              item.saveStatus == 0? <div className="body_detail"><Link  to={{pathname: '/app/drill/planEdit', query: { editItem: item }}}>
                  <Icon type='edit'   style={{cursor: "pointer",margin:'0 10px 0 -10px', color: '#999999'}}/><span style={{color: '#999'}}>编辑</span>
              </Link> </div> : null
          }
              
          {
              item.saveStatus == 0?  null:
              <div className="body_detail" style={{borderRight: "0"}}><Link to={{pathname: '/app/drill/planDetail', query: { editItem: item }}}>
          <Icon type='eye' style={{cursor: "pointer",margin:'0 10px 0 -10px', color: '#999999'}} /><span style={{color: '#999'}}> 查看</span> </Link> </div>
          }
         
         
          {
              item.saveStatus == 0?  <div className="body_delete"> <Popconfirm title='确认删除此计划信息?'
               onConfirm={()=>this.deletePlan(item)}
               >
  ...
  render() {
    const columns = [{
      ...
      key: 'key',
        render:(data)=>{
          return <span style={{color:'#1890ff',cursor: 'pointer'}} onClick={()=>this.queryDetail(data)}>查看详情</span>
        }
    }];    
    return (
