...
class ViewGridRaidRealTime extends Component{
    ...
    getGridTaskById(){//根据id请求数据
        let _this = this;
        return httpAjax('post',config.apiUrl+'/api/cmdMonitor/getGridTaskById',{"taskId":this.props.match.params.realID})
        .then((res)=>{
            if(res.code==0){
                this.setState({
                    detail:res.data,
    ...
    showAllAppTrochoidHis(taskType){//根据任务ID及任务类型 获取所有人员的轨迹信息
        let _this = this;
        return  httpAjax('post',config.apiUrl+'/api/cmdMonitor/showAllAppTrochoidHis',
                {taskId:this.props.match.params.realID,taskType})
                .then((res)=>{
                    if(res.code==0){
                        this.setState({
    ...
    qryTrochoidById(trackId,taskDetailId,userId){// 获取“后续”轨迹信息
        let _this = this;
                    ...
                    if(res.code==0){
                        ...
                        newAllPathsHis.forEach(ele=> {
                           if(ele.userId==userId){
                                if(!_.isEmpty(ele.pathsHis)){//当前人员已有轨迹数据拼接轨迹点
    ...
    polylineHandle(allPathsHis,flag){//画轨迹 
        let _this = this;
        allPathsHis.forEach((item,index)=>{
            //item 每个人的规矩
            _this['Polyline'+item.userId+item.taskDetailId]&&_this['Polyline'+item.userId+item.taskDetailId].setMap(null);
            ...
            //ite每个人多段轨迹
                ...
                ite.forEach((it,ind) => {
                    ...
                    if((inde==item.pathsHis.length-1)&&(ind==ite.length-1)){
                        _this['marker'+item.userId+item.taskDetailId]&&_this['marker'+item.userId+item.taskDetailId].setMap(null);
                        // //创建一个当前位置Marker
    ...
    if(!_.isEmpty(allPathsHis)){//该任务是否有人执行
        allPathsHis.forEach((item,index)=>{
        ...
        _this.timer = setInterval(() => {//定时获取轨迹信息
    ...
    render(){
        ...
        return(
            _this.state.allPathsHis&&<div className="gridRaidRealTime" style={{left:collapsed?"92px":"212px"}}>
               <div id="container"></div>
               <List
                    itemLayout="horizontal"
                    dataSource={this.state.allPathsHis}
                    renderItem={item => (
                    <List.Item onClick={this.itemHandle.bind(this)} extra={
                        <img
                          width={27}
                          alt="logo"
                          src={two}
                        />
                          ...
                          <span style={{margin:'0 15px',width:'20px',height:'20px',
                              backgroundColor:item.color,borderRadius:"50%"}}></span>
                        <List.Item.Meta
