...
class  FormCompomnent extends React.Component{
	...
	handleSubmit=(e)=>{
		...
		this.props.form.validateFields((err,values)=>{
			if(!err){
				...
				const params = {name: values.name, planDate: values.planDate.format('x'), vaccineType: values.vaccineType.join(',')}
				if(this.props.location.query) {
				
					params.id = this.props.location.query.record.id;
				}
				  httpAjax('post',config.apiUrl+'/api/vaccineRecord/savePlan', params).then((res)=>{
		      		if(res.code==0){
	...
	render(){
		...
		let name = '', planDate = '', vaccineTypename = '';
		if(this.props.location.query) {
									  ...
									  <Col xl={12} lg={12} md={24} sm={24} xs={24}>
							               	<FormItem label='计划名称' {...secondLayout} >
							               		{getFieldDecorator('name',{
							               			rules:[{required:true,message:'请填写计划名称'},{max:50,message:'计划名称长度不超过50'}],
