...
class HolidayTable extends Component{
      ...
      if (!error) {
        ...
        httpAjax('post',config.apiUrl+'/api/performanceCheck/cancelRank',{...row}).then((res)=>{
            ...
            if (index > -1) {
              ...
              this.setState({ dataSource: newData, visible: false,});
            } else {
 ...
 addInfo=()=>{
  ...
  render(){
    const {dataSource,loading,pagination,selectedRowKeys,checkDate}=this.state;
    ...
    const columns =[
      ...
      ,{
        ...
        render:(text,record,index)=>{
          return <div>
                    <Link  to={{pathname:'/app/performance/registerDetail', query: { record: record,checkDate:checkDate}}}>
