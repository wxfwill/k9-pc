...
class EditableCell extends React.Component {
  ...
  render() {
    ...
    const dutyUser=optionsArr&&optionsArr.map((item,index)=>{
      return <Option  key={index} value={item.userId+'&'+item.name+'&'+item.groupId+'&'+item.classId} >{item.name}</Option>
    })
    return (
      <div className="editable-cell">
        {
          ...
          this.props.index<8?  
              <div className="editable-cell-input-wrapper">
                      <Select defaultValue={value}  style={{width:'100%'}}  placeholder='请选择排班人员' onChange={this.selectChange}>
                        {dutyUser}
                      </Select>                  
                ...
                :
                <div className="editable-cell-input-wrapper"> 
                      <Input.TextArea placeholder="请输入备注信息！" defaultValue={value} onChange={(e) => {this.setState({value: e.target.value})}} autosize={{ minRows: 2, maxRows: 24 }} />               
                      <Icon type="check" className="editable-cell-icon-check"  onClick={this.check}  />
                </div>             
              : 
              this.props.index<8?  
