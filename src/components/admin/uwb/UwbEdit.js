...
class VideoInfo extends React.Component {
  ...
  handleSubmit = () => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        React.$ajax
          .postData('/api/equipmentInfo/saveUwbInfo', parms)
          .then((res) => {
            if (res.code == 0) {
  ...
  componentDidMount() {
    ...
    if (VideoId) {
      React.$ajax
        .postData('/api/equipmentInfo/uwbInfo', {id: VideoId})
        .then((res) => {
          if (res.code == 0) {
                    ...
                    <Col xl={12} lg={12} md={24} sm={24} xs={24}>
                      <FormItem label="标签编号:" {...secondLayout} hasFeedback>
                        {getFieldDecorator('code', {
                          rules: [
                            {required: true, whitespace: true, message: '请输入UWB标签编号'},
