...
import {Radio, List, Avatar, Skeleton, Row, Col, Icon, Spin, Button, Table, Card, Collapse ,Badge,Tag,Select,Modal,Form,Tabs,Input,message,DatePicker } from 'antd';
...
class _GridRaid extends Component{
  constructor(props){
    ...
    this.state= {
      ...
      showAllPeo: false, 
      radiusList:[],
      radiusList2:[{id: 5, name: '5公里', value: 3}, {id: 10, name: '10公里', value: 4}, {id: 20, name: '20公里', value: 5}, {id: 30, name: '30公里', value: 6}],
  ...
  componentDidMount() {
    ...
    this.getGridSearchSet().then(() => {
      ...
      this.TMap.showSelPeopleDlg=function(rect){ //点击搜索矩形区域,弹出对话框
        console.log(rect, 'mouseup')
        _this.setState({searchArea:rect});
        _this.dragOnePeople(rect);
        // _this.setState({selPeopleDlgVisible:true});
      }
      this.TMap.showAllPeople=function(event, rect, targetArea){ //点击搜索矩形区域,弹出对话框
  ...
  subGridSearchTask=() => {
      ...
      this.props.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          httpAjax('post',config.apiUrl+'/api/cmdMonitor/saveGridTask',{...params}).then((res)=>{         
  ...
  resetMapSearch = (parts) => { // 选择搜索半径 重新绘制搜索网格
    console.log(parts);
    const { radiusList } = this.state;
    // this.setState({parts: value});
    radiusList.forEach((item) => {
      if(item.parts==parts){
        this.setState({radius: item.radius, parts: parts}, ()=>{this.repaint()})
        // this.repaint();
      }
    })
    
  }
  showAllPeople = (event, rect, targetArea) => { //点击栅格 展示所有人员选择列表
    ...
    this.setState({showAllPeo: !this.state.showAllPeo, allPeopleTop: top, allPeopleLeft: left, targetEvent: event, curSearchRect: rect, currentArea: targetArea});
  }
  checkOnePeople = () => {
  ...
  dragOnePeople = (targetEvent, searchArea, currentArea) => { //栅格填充已选人员 
    let _this = this, rectIndex = searchArea.index;
    ...
    if(!_.isEmpty(selectOne)) {
      ...
      allPeoples.forEach((item, index) => { // 可选列表隐藏已选人员
  ...
  getGridSearchUsers(){ //获取下拉框中的用户列表
    var params={
      rid:Math.random()
    }

    var me=this;
    httpAjax('post',config.apiUrl+'/api/userCenter/gridSearchUser',{...params}).then((res)=>{
      me.selPeopleArr=res.data; //保存到当前对象中,后面要获取这个用户的id
      const selPeopleOptions=[];
      res.data.forEach((item,index) => {
        selPeopleOptions.push(<Option key={item.id} value={item.number}>{item.name}</Option>);
      })
  ...
  renderSelectNums = (id) => {  //更新展示该人员已选次数  
    let nums = 0;
  ...
  deleteSelectPeoSec = (item) => { //删除已选区域人员 sec
    console.log(item);
    let marker, label, {selectPeoples, allPeoples} = this.state,
    filter_markerList=[], 
    filter_labelList=[], 
    ...
    let curReactMarkers = this.reactMarkers[item.index], isOnly = false;
    if(curReactMarkers&&curReactMarkers.length==1){
      isOnly = true
    }

    this.markerList.forEach((mItem, index) => { // 对应的marker
    ...
    this.labelList.forEach((lItem, index) => { //对应的marker下的label
    ...
    curReactMarkers.forEach((rItem, index) => { //对应栅格里的人员
      if(rItem.id != item.id) {
        filter_curReactMarkers.push(rItem);
      }
    })

    selectPeoples.forEach((sItem, index) => { // 删除选中的人员 & 删除上报人员
      let hasFind=false;
  ...
  //  debugger
    ...
    selectPeoples.forEach((item, index) => { // 删除选中的人员
      let hasFind=false;
    ...
    allPeoples.forEach((allItem, index) => { //可选人员列表展示删除人员
      selectedRows.forEach((rowItem) => {
    ...
    this.markerList.forEach((mItem, index) => { // 删除地图标记
      let hasFind=false;
      selectedRows.forEach((rowItem) => {
  ...
  repaint = () => {
    ...
    this.TMap.drawingManager();//绘图功能
    this.onSearch();
    this.TMap.showSelPeopleDlg = (rect) => { //点击搜索矩形区域,弹出对话框
      this.setState({searchArea:rect});
      this.dragOnePeople(rect);
      // _this.setState({selPeopleDlgVisible:true});
    }
    this.TMap.showAllPeople = (event, rect, targetArea) => { //点击搜索矩形区域,弹出对话框
...
getGridSearchSet = () => { //2020 获取网格化搜捕配置
  return new Promise((reslove, reject) =>{
    httpAjax('post',config.apiUrl+'/api/cmdMonitor/gridSearchSet').then((res)=>{         
...
onSearch =(value)=>{
  ...
  render(){
    ...
    const { allPeopleH, allPeoples, selectOne, selectPeoples, showAllPeo, allPeopleTop, allPeopleLeft, showDeleteBtn, teamList, selectTeamId,reportUserList,radiusList, parts} = this.state;
    const title = <div className="card_title"><span>网格搜捕</span><span onClick={this.repaint}>重新绘制</span>&nbsp;</div>;
    const tabSearch = [<div className="tabSearch" style={{marginBottom: '10px'}} key="tab_1"><span style={{marginRight: '15px'}}>区域人员</span>
    <Input.Search  placeholder="姓名/警号" style={{ width: 200 }} enterButton onSearch={value => {this.getTeamPeople(null, value)}} /></div>,
    <div className="tabSearch" key="tab_2"><span style={{marginRight: '15px'}}>选择小队</span>
    <Select style={{width: 200, paddingBottom: '10px', }} dropdownStyle={{zIndex:'99999'}} value={selectTeamId} onChange={(value) => {this.setState({selectTeamId: value}); this.getTeamPeople(value)}}>
    {
        ...
        <div className="GridRaid" style={{left:collapsed?"92px":"212px"}}>
          <Spin indicator={antIcon} size="large" tip="数据加载中..." spinning={ this.state.loading } style={{position:'absolute',top:'50%',left:'50%',zIndex:'9999'}}/> 
            ...
            <Row gutter={24} id="container" >
                  <Card title={title} bordered={false} extra={<Tag color="#2db7f5" onClick={this.props.history.goBack}><Icon type="rollback" />返回</Tag>}  bordered style={{ width: 360,position:'absolute',top:'0',right:'0',zIndex:'9999', maxHeight:600, overflow: 'auto'}}>
                    ...
                    <Form horizontal="true">
                          <FormItem
                            ...
                            {getFieldDecorator('searchArea', {
                              ...
                              <Select placeholder="搜索半径" onChange={(value) => { this.resetMapSearch(value)  }}>
                              
                                {
                                  radiusList&&radiusList.map((item) => <Option key={item.radius} value={item.parts}>{item.name}</Option>)
                                }
                              </Select>
                            )}
                          </FormItem>
                          ...
                          <FormItem
                            ...
                            wrapperCol={{span: 14}}>
                             {getFieldDecorator('content',{
								               	rules: [{ required: true, message: '请输入任务内容...' },{ max: 1000, message: '任务内容长度不超过1000' }],
								               	initialValue:""
								               })(
                         ...
                         <FormItem
                            ...
                            {getFieldDecorator('combatType', {
                              ...
                              <Select placeholder="上报人员" onChange={(value) => { this.setState({ reportUserId: value }) }}>
                              
                                {
                                  reportUserList&&reportUserList.map((item) => <Option key={item.id} value={item.id}>{item.name}</Option>)
                                }
                              </Select>
                            )}
                         </FormItem>
                         ...
                         <FormItem
                            ...
                            labelCol={{span: 6}}
                            wrapperCol={{span: 14}}>
                            <Button type="primary" onClick={() => {this.setState({showAllPeo: !showAllPeo})}}>分配人员</Button>
                         </FormItem>                          
                      </Form>  
                      <Row>
                        <Col span="11"></Col>
                        <Col span="3"><Button type="primary" size="small" onClick={this.subGridSearchTask}>发布</Button></Col>
                      </Row>             
                      <Tabs defaultActiveKey="1">
                        <TabPane tab="区域人员" key="1">
                          <GridUserInfoTable isCheck={ false } selectPeoples={selectPeoples} contralDeleteBtn={(status) => {this.setState({showDeleteBtn: status})}} ref="child"/>
                        </TabPane>
                      </Tabs>  
                      <Row>
                        <Col span="14"></Col>
                          <Col span="4">
                          {
                            showDeleteBtn ? <Button type="primary" size="small" onClick={this.deleteSelectPeo}>删除</Button> : ''
                          }  
                          </Col>
                        <Col span="4"><Button size="small" onClick={this.clearAllSelect}>清空</Button></Col>
                      </Row>             
                </Card>
                  {
            ...
            <Row gutter={24} id="peo_container" >
              <div class="showAllPeoCard"
                style={{ display:showAllPeo?'block':'none', width: 335, height: allPeopleH,	overflow: 'hidden', position:'absolute',top: allPeopleTop,left: allPeopleLeft,zIndex:'9999'}}
              >
                ...
                >
                  <div className="avator_card" style= {{ padding: 5 }}>
                    <List
                      ...
                      renderItem={item => (
                        <List.Item
                          actions={[ <Icon onClick={() =>{ this.deleteSelectPeoSec(item) }} type="delete" key="delete" /> ]}
                        >
                  ...
                  <Tabs defaultActiveKey="1">
                    ...
                    {
                      allPeoples.map((t) => {
                        return (<div className="peo_item" key={t.id} onClick={() => {this.checkOnePeople(t)}} onMouseDown={() => {this.setState({selectOne: t})}}>
                        {this.renderSelectNums(t.id)}
                        <div className="cover_peo"></div>
                        <img src={ this.state.jc_path } />
                        <div className="peo_dec">{t.name}-{t.number}</div>
                      </div>)
                      })
                    }
                    </Row>
                ...
                <Card
                  ...
                  actions={[
                    <Radio.Button onClick={() => {this.setState({showAllPeo: false})}} value="large">确定</Radio.Button>
                  ]}
                >
                </Card>
              </div>
