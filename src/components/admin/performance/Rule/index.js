...
class PerformanceRuleTable extends React.Component{
	constructor(props){
  	...
  	fetch(params = {pageSize:this.state.pageSize,currPage:this.state.currPage}){
	    ...
	    httpAjax('post',config.apiUrl+'/api/basicData/performanceCheckType',{...params,id:1}).then((res)=>{
	      const pagination = { ...this.state.pagination };
	      pagination.total =res.totalCount;
	      pagination.current = res.currPage;
          pagination.pageSize = res.pageSize;
	      this.setState({menuList:res.data,current:'id_'+res.data[0].id,performanceId:res.data[0].id,loading:false,pagination})
		...
		return(
			<div>			
                 <Menu
                    ...
                    mode="horizontal"
                >
                    {menuList.length>0?menuList.map((item) =>  <Menu.Item key= {'id_'+item.id} >
                        {item.name}
                    </Menu.Item>):null}
                </Menu>
