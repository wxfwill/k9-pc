...
class SearchForm extends React.Component {
  ...
  getTrainer = (name="") => httpAjax('post', config.apiUrl+'/api/userCenter/getTrainer',{name}).then(res => {
    if(res.code == 0) {
  ...
  render() {
    ...
    const dogBreedOption= dogBreed&&dogBreed.map((item,index)=>{
              ...
              <FormItem label='带犬员' {...thirdLayout}>
                {getFieldDecorator('trainerIds', {
                  ...
                  <Select
                    ...
                    //style={{ width: '100%' }}
                  >
                  {trainerList.map((t) => <Option value={t.id} key={t.id}>{t.name}</Option>)}
                  </Select>
                )}
              </FormItem>
            </Col>
