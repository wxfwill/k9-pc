...
const BeardComponent = ({location, menus, history}) => {
  const breads = [];
  let showBack = false;
  const pageArr = ['Schedule', 'Detail', 'AddUser', 'UserData', 'EditUser', 'Add', 'View', 'Edit', 'Todo'];
  const isntIndex = !location.pathname.includes('index');
  const isMap = !location.pathname.includes('grid');
  let cacheKey = '';
  const jumpObj = {
    Schedule: '智能排班',
    Detail: '详情',
  ...
  return (
    <div style={{padding: '10px 25px 10px 24px', letterSpacing: '3px'}}>
      {isntIndex && isMap ? (
        <div>
          ...
          <Breadcrumb style={{float: 'left', marginRight: '15px'}}>
            {breads.map((item, key) => {
              return <Breadcrumb.Item key={key}>{item.name && item.name}</Breadcrumb.Item>;
            })}
          </Breadcrumb>
          {showBack ? (
            <Tag color="#2db7f5" onClick={history.goBack}>
              <Icon type="rollback" />
              返回
            </Tag>
          ) : null}
