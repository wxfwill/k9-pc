...
class SearchForm extends React.Component {
  ...
  componentWillMount() {
    ...
    const LevelOption = React.$ajax.postData('/api/trainingSubject/getAllTrainLevel');
        ...
        <Row gutter={24}>
          ...
          <Col xl={8} lg={12} md={12} sm={24} xs={24}>
            <FormItem label={'训犬人员'} {...thirdLayout}>
              {getFieldDecorator('userName')(<Input placeholder="训犬人员" />)}
            </FormItem>
          </Col>
          <Col xl={8} lg={12} md={12} sm={24} xs={24}>
            <FormItem label={'时间'} {...thirdLayout}>
              {getFieldDecorator('trainDate')(<DatePicker />)}
            </FormItem>
          </Col>
        </Row>
        <Row>
