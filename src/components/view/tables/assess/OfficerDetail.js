...
class OfficerDetail extends Component {
	...
	fetch(params){
    httpAjax('post',config.apiUrl+'/api/trainCheck/detail',{key:params}).then((res)=>{
      const { data } = res;
      this.setState({
      	title:data.title,
      	baseArr:[{
				  	...
				  	<Collapse defaultActiveKey={['1','2']}>
					    <Panel showArrow={false} header={this.baseHeader()} key="1">
	          			...
	          			<Col xxl={16} xl={24} lg={24} md={24} sm={24} xs={24} >
	          				<Table loading={this.state.loading} columns={this.getBaseColumns()} dataSource={this.state.baseArr}  pagination={false} bordered/>
	          			</Col>
	          		</Row>
					    </Panel>
					    <Panel showArrow={false} header={this.checkHeader()} key="2">
					      <Row gutter={24}>
	          			<Col xxl={16} xl={24} lg={24} md={24} sm={24} xs={24} >
	          				<Table loading={this.state.loading} columns={this.getCheckColumns()} dataSource={this.state.checkArr}  pagination={false} bordered/>
	          			</Col>
	          		</Row>
					    </Panel>
				  	</Collapse>
