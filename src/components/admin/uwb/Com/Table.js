...
class DogTable extends Component{
  ...
  fetch(params = {pageSize:this.state.pageSize,currPage:this.state.currPage}){
    ...
    httpAjax('post',config.apiUrl+'/api/equipmentInfo/uwbPageList',{...params}).then((res)=>{
        ...
        render:(text,record,index)=>{
          ...
          return <div>
            <span style={{cursor: "pointer",color:'#1890ff'}} onClick={()=>this.viewDetail(record)}>
              <Link to={{pathname:'/app/equipment/uwbDetail', query: { VideoId: record.id,targetText:'查看' }}}>查看</Link>
            </span>
