                ...
                {getFieldDecorator('trainLevel')(
                  <Select placeholder="训练阶段">
                    {this.state.LevelOption.map((item,index)=>{
                        if(item.trainLevel == ""){
                            return(<Option key={item.trainLevel} value={item.trainLevel}>全部</Option>)
                        }else{
                            return(<Option key={item.trainLevel} value={item.trainLevel}>{optionArr[item.trainLevel-1]}</Option>)
                        }
                    })}
                  </Select>
                )}
