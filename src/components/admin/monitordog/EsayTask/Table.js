...
import { firstLayout } from 'components/view/common/Layout';
...
class DogTable extends Component{
  ...
  fetch = (params = {pageSize:this.state.pageSize,currPage:this.state.currPage}) => {
    ...
    httpAjax('post', config.apiUrl+'/api/outdoorTask/listPlanData',params).then((res) => {
      this.setState({dataSource:[...this.state.dataSource,...res.data.list],loading:false,totalPage: res.data.totalPage})
    })
  }
  ...
  render(){
    ...
    return (
        ...
        <div style={{marginBottom:'20px'}}>
          <Link to={{pathname:'/app/monitoring/itinerancyAdd', query: {targetText:'新增' }}}><Button type="primary">新增任务</Button></Link>
        </div>
        <div>
        {
          dataSource.map((item) =>  
            <Card 
              ...
              // <span><Icon type="edit" style={{marginRight: 7}} />修改</span></Link>,
                <span >
                  ...
                  <Icon type="delete" style={{marginRight: 7}} />删除</Popconfirm>
                </span>
              ]}
            >
              <Meta
                ...
                description={[
                  <div key={item.id+'01'} style={{ wordBreak: 'keep-all',whiteSpace: 'nowrap',overflow: 'hidden',textOverflow: 'ellipsis'  }}>
                    <Tooltip autoAdjustOverflow='false'  placement="topLeft" title={item.userNames.join(',')}>  人员：{item.userNames.join(',')}</Tooltip>
                  </div>,
                  <div key={item.id+'02'} className="item">
                      <span>上报人员：</span><span>{item.reportUserName ? item.reportUserName : '----'}</span>
                  </div>,
                  <Tooltip key={item.id+'03'} autoAdjustOverflow='false'  placement="topLeft" title={item.content}> 
                    <div style={{ wordBreak: 'keep-all',whiteSpace: 'nowrap',overflow: 'hidden',textOverflow: 'ellipsis'  }}>勤务说明：
                       <p style={{marginBottom:0}}>{item.content}</p>
                    </div>
                  </Tooltip>,
                  <div key={item.id+'04'} className="item">
                      <span>发布时间：</span><span>{item.publishDate ? moment(item.publishDate).format('YYYY-MM-DD HH:mm') : '----'}</span>
                  </div>,
        ...
        {
            this.state.dataSource.length == 0 ? <div style={{ textAlign: 'center', color:'#999', marginTop: 12, height: 32, lineHeight: '32px' }}> 暂无数据</div>:''
        }
        </div>
	    </div>
    )
