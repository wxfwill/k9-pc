          ...
          <Collapse defaultActiveKey={['1', '2']}>
            <Panel showArrow={false} header={this.getTitle('报表形式')} key="1">
              <Form onSubmit={this.handleSearch}>
                <Col xl={8} lg={24} md={24} sm={24} xs={24}>
                  <FormItem label="查询类型" {...thirdLayout}>
                    {getFieldDecorator('typeId', { initialValue: typeId })(
                      <Select placeholder="查询类型">{typeOption}</Select>
                    )}
                  </FormItem>
                </Col>
                ...
                <Button onClick={() => this.getRangDate('week')}>本周</Button>&nbsp;&nbsp;
                <Button onClick={() => this.getRangDate('month')}>本月</Button>
              </Form>
                  ...
                  TabList.map((item) => (
                    <TabPane tab={item} key={item + ''}>
                      <div style={{ marginBottom: '12px' }}>
                        {tabKey} | 出动警力： <span style={{ color: '#52c41a' }}>{userNum || 0}</span>人 | 携犬出勤：{' '}
                        <span style={{ color: '#52c41a' }}>{dogNum || 0}</span>头 |
                      </div>
                      <Table
                        loading={loading}
                        columns={baseColumns}
