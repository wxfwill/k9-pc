...
class CallTable extends React.Component {
  ...
  fetch(params = {pageSize:this.state.pageSize,currPage:this.state.currPage}){
    ...
    httpAjax('post',config.apiUrl+'/api/attendance/listPage',{...params}).then((res)=>{
    ...
    const columns = [
      ...
      },
      {
        ...
        render:(photoNames,record,index)=>{
          return  <img  onClick={_this.showPhoto.bind(this,photoNames)} key={index} src={`${config.apiUrl}/api/attendance/img?fileName=${photoNames[0]}`} style={{height:'30px',width:'30px',marginRight:'8px'}}/>
        }
      }];
    return columns;
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        >
         <Carousel autoplay >
         {photoNames&&photoNames.length>0?photoNames.map((file,index)=>
                   <div ><img key={index} src={`${config.apiUrl}/api/attendance/img?fileName=${file}`} /></div>
                  ):<div >{'暂无就图片信息'}</div>
                  }
