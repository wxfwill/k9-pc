...
class HolidayTable extends Component{
  ...
  fetch(params = {pageSize:this.state.pageSize,currPage:this.state.currPage,checkDate:this.state.checkDate}){
    ...
    httpAjax('post',config.apiUrl+'/api/performanceCheck/listPerformanceCheckRank',{...params}).then((res)=>{
      ...
      ,{
        ...
        render:(text,record,index)=>{
          return <div>
           ...
           <div>
            <Link  to={{pathname:'/app/performance/registerEdit', query: { record: record,checkDate:checkDate }}}>
              <span  style={{cursor: "pointer",color:'#1890ff'}} ><Icon type='edit' style={{margin:'0 10px', }}  />编辑</span>
            </Link>          
           
          
              <span onClick={() => this.showModal(record.id)}  style={{cursor: "pointer",color:'#1890ff'}} ><Icon type='close-circle-o' style={{margin:'0 10px', }} />取消当月资格</span>
            </div>:
            <div ><Link  to={{pathname:'/app/performance/registerDetail', query: { record: record,checkDate:checkDate }}}>
