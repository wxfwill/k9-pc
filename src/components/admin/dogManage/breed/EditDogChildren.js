...
class EditDogChildren extends Component {
    ...
    getDogChildren = () => {
        ...
        httpAjax('post', config.apiUrl+'/api/breed/reproduceDetail', {id}).then((res) => {
            if(res.code == 0) {
                ...
                const newDiffs = diffs&&diffs.map((t, i) => {
    ...
    updatePupName = (item) => {
        this.props.form.validateFields((err,values)=> {
                ...
                httpAjax('post', config.apiUrl+'/api/breed/updatePupName', {pupId: item.id, pupName: values[item.id+'name']}).then((res) => {
                    if(res.code == 0) {
                       message.info('修改成功！');
                    } else {
                        message.error('修改失败！')
    ...
    saveMeasureRecord = (item) => {
        ...
        this.props.form.validateFields((err,values)=> {
            let newWeightList, successMsg='', errorMsg='';
            if(pupWeightList) {
                successMsg="修改成功！";
            ...
            httpAjax('post',  config.apiUrl+'/api/breed/saveMeasureRecord',{list: newWeightList}).then((res) => {
                if(res.code == 0) {
    ...
    renderOperation =(record, index) => {
        ...
        if(index.diffs) {
          ...
          return  edit && editDate == index.date?
            <Button type="primary" onClick={() => {this.saveMeasureRecord(index)}} >保存</Button>:
            <Button type="primary" onClick={() => this.clickEdit(index)}>编辑</Button>
        }
    ...
    render() {
        ...
        console.log(this.state);
          ...
          const addColumns = pupInfos.map((item) => {
            return {
                ...
                dataIndex: item.id,
                key: item.id,
                render: (record, index) =>edit && editDate == index.date? <Input  defaultValue={this[item.id+'weight']} onChange={(e) => {this[item.id+'weight'] = e.target.value}} />: record,
            }
          })
          ...
          const columns = [{
            ...
            render:  (record, index) => edit && editDate == index.date? getFieldDecorator('opDate',{
                rules: [{ required: true, message: '请输入' }],
                initialValue:  record && record != 'ACKS' ?moment(record):''
                })(<DatePicker   />): record && record != 'ACKS'?moment(record).format('YYYY-MM-DD'):'',
          }, 
          ...addColumns,
          {
            title: '平均增重(kg)',
            ...
            dataIndex: 'stdWeight',
            key: Math.random(),
            render: (record, index) =>edit && editDate == index.date?<Input defaultValue={this.stdWeight} onChange={(e) => this.stdWeight = e.target.value} />:record,
          }, {
            title: '日龄',
            dataIndex: 'days',
            key: Math.random(),
            ...
            dataIndex: 'event',
            key: Math.random(),
            render:(record, index) => edit && editDate == index.date?<Input defaultValue={this.changeEvent} onChange={(e) => {this.changeEvent= e.target.value}}  />:record ,
          },{
            title: '操作',
            dataIndex: 'address',
            key: Math.random(),
        ...
        return <div className="edit_dog_child" >
        <Form>
            <Table 
          ...
          <Button onClick={this.addRow}  type="primary" style={{position: 'absolute',right: '51px',top: '-40px'}}>添加</Button>
          <Tag color="#2db7f5" style={{left: '0px', position: 'absolute',top: '-37px'}} onClick={this.props.history.goBack}><Icon type="rollback" />返回</Tag>
        </div>
    }
}
