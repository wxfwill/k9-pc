...
class DogTable extends Component{
  ...
  fetch(params = {pageSize:this.state.pageSize,currPage:this.state.currPage}){
    ...
    httpAjax('post',config.apiUrl+'/api/vaccineRecord/listVaccinePlan',{...params}).then((res)=>{
  ...
  viewDetail=(record)=>{
    ...
    history.push({pathname:`/${role}/dog/preventionView`,
      query: {  
        id: record.id,
        targetText:'查看' 
      }  
    ...
    const columns =[
      ...
      // }, /app/dog/preventionTodo
      {
        ...
        render:(text,record,index)=>{
          return <div>
          {
            record.status == 1?<span style={{cursor: "auto",color:'rgb(208, 215, 222)',display:'inline-block',marginRight:'10px'}}>
              执行
            </span>:  <Link  to={{pathname:`/${role}/dog/preventionTodo`, query: { record: record,targetText:'编辑' }}}>
           <span style={{cursor: "pointer",color:'#1890ff',display:'inline-block',marginRight:'10px'}}>
            ...
            {
              record.status == 1?<Icon type='edit'   style={{cursor: "auto",color:'rgb(208, 215, 222)',margin:'0 10px'}}/> : <Link  to={{pathname:`/${role}/dog/preventionEdit`, query: { record: record,targetText:'编辑' }}}>
              <Icon type='edit'   style={{cursor: "pointer",color:'#1890ff',margin:'0 10px'}} onClick={()=>this.editInfo(record)}/>
            </Link>  
            }
