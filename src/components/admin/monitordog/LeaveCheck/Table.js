...
class DogTable extends Component{
  ...
  erifyLeaveApply = (record, verify) => {
    this.props.form.validateFields((err, values) => {
      httpAjax('post', config.apiUrl+'/api/leaveRecord/verifyLeaveApply', {opinion: values.opinion, verify, id: record.id}).then((res) => {
        if(res.code == 0) {
    ...
    const columns =[
      {
        ...
        render:(text,record,index)=>{
          // console.log(text, record, index)
          return <Button type="primary" disabled={record.record == 2?false: true} onClick={() => this.showModal(text,record,index)}>审批</Button> 
        }
      }
    ]
            ...
            <Col xl={24} lg={24} md={24} sm={24} xs={24} >
              <Form.Item label='审批意见' {...firstLayout}>
                {getFieldDecorator('opinion')(
                  <Input.TextArea placeholder="审批意见" />
                )}
              </Form.Item>
            </Col>
          </Row>
