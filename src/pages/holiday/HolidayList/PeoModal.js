...
import { firstLayout,secondLayout,thirdLayout} from 'components/common/Layout';
...
class PeoModal extends Component{
  ...
  getData(address='/api/userCenter/getCombatStaff'){
    ...
    httpAjax('post',config.apiUrl+address,{type:this.props.typeId,leaveYear:this.props.leaveYear}).then((res)=>{
  ...
  changeUserData(res){
    ...
    res.data.length>0 && res.data.forEach((item,index)=>{
      Array.prototype.push.call(userData,{
        key:keyMark+item.id,
        name:item.name,
        description:(typeof item.remark=='undefined'||item.remark==""||typeof item.remark=='object')?"":`(${item.remark})`,
  ...
  selectChangeData(params){
    httpAjax('post',config.apiUrl+params.address,{[params.paramsKey]:params.paramsValue}).then((res)=>{
      this.changeUserData(res);
    }).catch((error)=>{
      console.log(error);
    })
          ...
          <Row gutter={24}>
            <Col xxl={10} xl={10} lg={10} md={24} sm={24} xs={24}>
              <FormItem label={'分局：'} {...secondLayout}>
                <Select placeholder="选择分局" disabled={!this.state.limitBranch} onSelect={this.SelectBranch.bind(this)}>
                  {this.state.BranchArr.length>0&&this.state.BranchArr.map((item,index)=>{
                    return(
            ...
            <Col xxl={10} xl={10} lg={10} md={24} sm={24} xs={24}>
              <FormItem label={'分队：'} {...secondLayout}>
                <Select placeholder="选择分队" disabled={this.state.limitBranch} onSelect={this.SelectTeam.bind(this)}>
                  {this.state.TeamArr.length>0&&this.state.TeamArr.map((item,index)=>{
                    return(
                      <Option key={item.id} value={item.id}>{item.name}</Option>
                    )                      
                  })}
            ...
            <Col xxl={4} xl={4} lg={4} md={24} sm={24} xs={24}>
              <FormItem {...secondLayout} className="fl-r">
                <Switch checkedChildren="分局" unCheckedChildren="分队" defaultChecked className="fl-r" onChange={this.handleSwitchChange.bind(this)}/>
              </FormItem>
            </Col>
          </Row>
        </Form>
