...
class SmartTable extends React.Component {
  ...
  componentDidMount() {
    this.fetch(
      {startDate: this.props.filter.startTime, endDate: this.props.filter.endTime, currPage: this.state.currPage},
      true
    );
  }
  ...
  fetch(params, isFirst) {
    const _this = this;
    const reqUrl = isFirst
      ? config.apiUrl + '/api/onDuty/genDutyData'
      : config.apiUrl + '/api/onDuty/getDutyDataByPage';
  ...
  handleData(data, callback) {
    ...
    const dateMsg = resData.dutyDates.splice(0, 1);
    const titleArr = ['带班领导', '值班', '副班', '值班辅警', '消毒人员', '备注'];
    ...
    const dutyData = [mondayArr, tuesdayArr, wednesdayArr, thursdayArr, fridayArr, saturdayArr, sundayArr];
      ...
      },
      {
        ...
        render: (text, record, index) => {
          ...
          if (typeof text.sterilizeLeader !== 'undefined') {
            const {sterilizeLeader, sterilizeEmployee, sterilizeAuxiliary} = text;
  ...
  render() {
    ...
    const loadMore = showLoadingMore ? (
      <div style={{textAlign: 'center', marginTop: 12, height: 32, lineHeight: '32px'}}>
        {loadingMore && <Spin />}
        {!loadingMore && (
          <Button onClick={this.onLoadMore}>{this.state.noMoreData ? '没有更多数据' : '加载更多...'}</Button>
        )}
      </div>
    ) : null;
    return (
