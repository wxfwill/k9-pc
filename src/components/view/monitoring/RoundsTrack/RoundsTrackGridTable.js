...
class SearchForm extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(type == 'duty'){
      taskType=2
		...
		if(socketMsg&&socketMsg.msgType == 'taskStatus' && socketMsg.data.id==taskID && taskType==socketMsg.data.type) {
     data.map((item,index)=>{
  ...
  fetchTaskInfo(params){
    ...
    httpAjax('post',config.apiUrl+'/api/dailyPatrols/getDailyPatrolsById',{id:params}).then((res)=>{
  ...
  fetchTrochoid=(params)=>{
    ...
    httpAjax('post',config.apiUrl+'/api/cmdMonitor/showAppTrochoidHis',{...params,...this.state.filter}).then((res)=>{
      me.setState({loading:false});
  ...
  getNowTrochoid=(data)=>{
    ...
    httpAjax('post',config.apiUrl+'/api/cmdMonitor/showAppTrochoid',{lastPointTime:trochoiInfo.lastPointTime,taskType:trochoiInfo?trochoiInfo.taskType:'',taskDetailId:trochoiInfo?trochoiInfo.taskDetailId:''}).then((res) => {
  ...
  render() {
    const columns = [{
      ...
      render: (state,record)=>{
        ...
        return this.props.type == 'duty'? <span  style={{cursor: "pointer",color:'#1890ff'}} onClick={(e)=>this.props.showVideo(e,record.taskDetailId)} ><Icon type='video-camera' style={{margin:'0 10px', }} /></span>:'--'
      },
    }];
    ...
    return (
      ...
      >
        ...
        <Table style={{ maxHeight: visibleHeight-250,overflowY:'scroll'}} loading={this.state.loading} columns={columns} dataSource={this.state.data} pagination={false} 
             onChange={this.handleTableChange} onRowClick={this.handonRowClick} size="small"/>
      </div>
    );
  }
}
