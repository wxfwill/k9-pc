...
const mockXl = [{
    bgColor: '#49a9ee', name:'训练场', num: '--', unit: '头', percent:'--%'
}, {
    bgColor: '#98d87d', name:'室内', num: '--', unit: '头', percent:'--%'
}, 
]
const mockJb = [{
    bgColor: '#49a9ee', name:'已处理', num: '0', unit: '条', percent:'--%'
}, {
    bgColor: '#ffd86e', name:'待处理', num: '0', unit: '条', percent:'--%'
}, 
]

const mockDf = [{
    name: '小明', score: '100', ranking: '1'
}, {
    name: '小网', score: '99', ranking: '2'
}, {
    name: '小网', score: '98', ranking: '2'
}, {
    name: '小网', score: '97', ranking: '2'
}]
...
class NewIndex extends Component {
    ...
    componentDidMount() {
        ...
        setTimeout(() => { socketon(this.socketOnmessage);},500)
    ...
    socketOnmessage =  (resData) => { 
        ...
        if(resData.code == 0 && resData.msgType == "locationMap") {
            ...
            resData.data.dogOnlineList.forEach((item) => {
                ...
                qq.maps.event.addDomListener(label, "mouseout",(e) => {self.setState({ mouseoverPoint:{}, mouseover: false,})});

                self.dogPoints.push(label);
            });
           self.baseList = resData.data.bases.map((item) => {
               return self.drawMapLabel(item, item.dogNumber == 1?'':`<div class="dog_base"><div class="base_dec"><div>${item.areaName}</div><div>${item.dogNumber?item.dogNumber:''}</div></div></div>`)
            })
        }
    }; 
    ...
    renderLabel = (list) => {
        return list && list.map((item, index) => {
            return <div  key={index+'k9_label'} className="k9_label">
    ...
    renderDogCard = (list) => {
        return list && list.map((item, index) => {
            return <Link to={{pathname: '/app/dog/info', query: {type: item.type}}} key={index+'doglist'}>
            <div  className="k9_card" style={{background: item.bgColor}}>
                <span className="k9_card_img"></span>
                <div className="card_title">{item.title}</div>
                <div className="dog_num"><span>{item.num}</span><i>（头）</i></div>
        </div></Link>
        });
    }
    renderCarousel = (list) => {
      return  <Carousel autoplay vertical="true">
    ...
    getDogCureData = () => {
        httpAjax('post',config.apiUrl+'/api/overView/treatmentSituation').then((res) => {
            if(res.code ==0) {
                ...
                const cureCycleList = [{
                    bgColor: '#49a9ee', name:'已处理', num: type1, unit: '条'
                }, {
                    bgColor: '#ffd86e', name:'待处理', num: type2, unit: '条'
                },];
                let cureTips = [];
                tips.forEach((item, i)=> {
                    if(i%2 == 0) {
    ...
    getTrainSituation = () => {
        httpAjax('post', config.apiUrl+'/api/train/getTrainSituation').then((res) => {
            if(res.code == 0) {
            const newData =  [{
                    bgColor: '#49a9ee', name:"基地内", num: res.data.indoorNum, unit: '头', percent:res.data.indoorNum==0 ? 0:(100*(res.data.indoorNum /  (res.data.indoorNum+ res.data.outdoorNum))).toFixed(0)+'%',
                }, {
                    bgColor: '#98d87d', name:"基地外", num:  res.data.outdoorNum, unit: '头', percent:res.data.outdoorNum==0 ? 0:(100*(res.data.outdoorNum /  (res.data.indoorNum+ res.data.outdoorNum))).toFixed(0)+'%',
                },];
                let cureTips = [];
    ...
    getDutyData = () => {
        httpAjax('post',config.apiUrl+'/api/onDuty/getTodayOnDuty').then((res) => {
         const dutyList =   [{label: '带班领导', value: res.data.onDutyLeaderName},{
                label: '值班', value: res.data.onDutyUserName
            },{
                label: '值班辅警', value:res.data.onDutyPoliceName
            },{
                label: '值班中队', value:res.data.onDutyTeam
            }];
    ...
    render () {
        ...
        const { rankData, cureCycleList, cureTips, dutyList, dogsInfo, mouseoverPoint, dogDrill, mouseover, trainSituationData} = this.state;
        const mouseOverData = [{
            key: 1,
            label: '犬名',
        ...
        return <div className="newIndex">
                <div className="index_map">
                    ...
                    <div className="mouseover_dev" style={{display: mouseover?'block': 'none',
                        left: `${mouseoverPoint.pixel&&mouseoverPoint.pixel.x}px`, top: `${mouseoverPoint.pixel&&mouseoverPoint.pixel.y}px`,}}>
                        {
                ...
                <div className="index_footer">
                    ...
                    <Card title="绩效考核排名" style={{flex: 1,  marginRight: 15}}  bodyStyle={{padding: '0', }}>
                        <div style={{height: 254, overflow: 'auto', padding: '0 24px', marginbottom: 1}}> 
                            <Table pagination={false}  columns={columns} dataSource={rankData} />
                        </div>
